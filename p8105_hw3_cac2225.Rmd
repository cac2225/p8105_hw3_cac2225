...
ggplot(orders_count, aes(fct_reorder(aisle, desc(total_order_count)), total_order_count, fill = department)) + 
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), text = element_text(size = 5)) + 
  labs(
...
A subset of data is generated, counting the total number of items ordered from each aisle, using the group_by and summarise functions. Using ggplot, the subset of data is plotted into a bar chart, with the aisles on the x axis and the total orders count on the y axis. The aisles are ordered from most to least orders, and with different colors representing different departments. The aisle that received the most orders is fresh vegetables with approximately 150,000 orders, and the aisle withthe least orders is beauty.
